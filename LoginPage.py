...
class LoginPage:
    def __init__(self, window):
        ...
        # ========================================================
        # ========================================================================
        self.txt = "Biometric Authenticated Voting System"
        self.heading = Label(self.lgn_frame, text=self.txt, font=('Times new Roman', 25, "bold"), bg="#040405",
                             fg='white',
                             bd=5,
                             relief=FLAT)
        self.heading.place(x=175, y=30, width=600, height=50)

        # ========================================================================
        # ============ Left Side Image ================================================
        # ========================================================================
        self.side_image = Image.open('images\\vector.jpg')
        photo = ImageTk.PhotoImage(self.side_image)
        self.side_image_label = Label(self.lgn_frame, image=photo, bg='#040405')
        self.side_image_label.image = photo
        self.side_image_label.place(x=150, y=180)
        ...
        self.forgot_button = Button(self.lgn_frame, text="Forgot Password ?",
                                    ...
                                    , borderwidth=0, background="#040405", cursor="hand2",command=self.change)
        ...
        self.signup_button_label = Button(self.lgn_frame, image=self.signup_img, bg='#98a65d', cursor="hand2",
                                          borderwidth=0, background="#040405", activebackground="#040405",command=self.reg)
    ...
    def hide(self):
        ...
        self.password_entry.config(show='*')
    def reg(self):
        os.system('python reg_page.py')
        sys.exit()
    def change(self):
        os.system('python changepass.py')
        sys.exit()
    def login(self):
        try:
            un=self.username_entry.get()
            ps=self.password_entry.get()
            cursor.execute("SELECT Password FROM reg_users WHERE Email = ?", (un,))
            em=cursor.fetchone()
            if em[0]==ps:
                ...
                self.det.insert(0, "Check username or password")
                self.det.config(state='disabled')
                self.det.place(x=300, y=559, width=300, height=60)
        except:
            self.det = Listbox(bg='#040405', font=("yu gothic ui", 13, "bold"), fg='#FF0000', borderwidth=10)
            self.det.insert(0, "Entry not found")
            self.det.config(state='disabled')
            self.det.place(x=300, y=559, width=300, height=60)
