...
class Regpage():
    def __init__(self,window):
        ...
        self.otpbut.place(x=400, y=309)
        self.reg=Button(self.sidebar_frame,text='Register', font=("yu gothic ui", 13, "bold"), width=40, bd=0,
                         bg='#228B22', cursor='hand2', activebackground='#3047ff', fg='white',command=self.add)
        self.reg.place(x=100,y=389)
    ...
    def add(self):
        ...
        if(self.OTP==int(self.otp.get())):
            try:
                if ps==rps:
                    cursor.execute('INSERT INTO reg_users (Name,Email,Password) VALUES (?,?,?)',(nm,em,ps))
                    connection.commit()
                    self.det = Listbox(bg='#040405', font=("yu gothic ui", 13, "bold"), fg='#FF0000', borderwidth=10)
                    self.det.insert(0, "Registration Successful")
                    self.det.config(state='disabled')
                    self.det.place(x=420, y=509, width=300, height=60)
            except:
                self.det = Listbox(bg='#040405', font=("yu gothic ui", 13, "bold"), fg='#FF0000', borderwidth=10)
                self.det.insert(0, "Registration Failed")
                self.det.config(state='disabled')
                self.det.place(x=420, y=509, width=300, height=60)
            os.system('python Loginpage.py')
            sys.exit()
